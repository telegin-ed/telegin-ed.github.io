...
## Часть 1. Введение в распределенную трассировку

### Глава 1. Отслеживание в современных приложениях

#### Почему логов и счетчиков недостаточно
...
#### Трассировка - строительные блоки
...
#### Внутрипроцессная передача (In-process propagation)
...
#### Обеспечение согласованности и структуры
...
### Глава 2. Встроенные возможности мониторинга в .NET

#### Простой пример приложения

##### Корреляция журналов (log correlation)
...
##### Логирование по требованию с помощью dotnet-monitor
...
#### Мониторинг с помощью счетчиков среды выполнения
...
#### Автоматическое накопление метрик с помощью OpenTelemetry
...
#### Обзор автоматически сгенерированной телеметрии
...
### Глава 3. Экосистема наблюдаемости (observability) .NET

В главе широко используются сервисы Azure, AWS, поэтому обзор главы будет с существенным сокращением.

#### Использование инструментария для популярных библиотек
...
##### Добавление инструментария в приложение
...
#### Использование инфраструктуры (Dapr)
...
##### Настройка наблюдаемости с Dapr
...
#### Настройка serverless окружения
...
### Глава 4. Низкоуровневый анализ производительности с помощью утилит диагностики
...
#### Обзор общих проблем производительности
...
#### Использование утилит диагностики в продакшене
...
## Часть 2. Применение инструментария в .NET приложениях

### Глава 5. Конфигурация и слой управления

#### Управление стоимостью профилирования с помощью сэмплирования
...
#### Обогащение (enriching) и фильтрация телеметрии
...
##### Обработчик спанов (span processor)
...
#### Кастомизация контекста (для легаси, customizing context propagation)
...
#### Обработка конвейера с использованием OpenTelemetry Collector
...
#### Трассировка с использованием System.Diagnostics или обертки OpenTelemetry API
...
##### Трассировка с помощью System.Diagnostics
...
###### Экспорт активности через OpenTelemetry
...
###### Прослушивание активности через ActivityListener
...
##### Трассировка с помощью обертки OpenTelemetry API
...
### Глава 7. Добавление пользовательских метрик
...
#### Метрики в .NET — прошлое и настоящее
...
#### Использование асинхронного гейджа (gauge)
...
### Глава 8. Запись структурированных и коррелированных логов
...
#### Сбор логов с помощью OpenTelemetry
...
#### Управление затратами на логирование
...
## Часть 3. Наблюдаемость для типичных облачных сценариев

### Глава 9. Лучшие практики наблюдаемости в облаке
...
#### Примеры применения телеметрии в разных сценариях
...
##### Семантические конвенции для HTTP-запросов
...
#### Общие соображения по инструментированию
...
### Глава 10: Трассировка сетевых вызовов
...
#### Инструментирование клиентских вызовов
...
#### Инструментирование серверных вызовов
...
#### 📡 Инструментирование потоковых вызовов (Streaming Calls)
...
#### Исследование проблем (Наблюдаемость в действии)
...
### Глава 11. Инструментирование сценариев обмена сообщениями
...
#### Наблюдаемость в сценариях обмена сообщениями
...
#### Инструментирование потребителя
...
##### Трассировка операций потребителя
...
###### Продолжительность, пропускная способность и частота ошибок
...
###### Измерение задержки потребителя
...
#### Инструментирование сценариев пакетной обработки
...
##### Пакетирование на уровне транспорта
...
#### Анализ производительности в сценариях обмена сообщениями
...
### Глава 12. Инструментирование вызовов баз данных
...
#### Инструментирование вызовов баз данных
##### Семантические соглашения OpenTelemetry для баз данных
##### Реализация трассировки
#### Трассировка вызовов кэша
##### Инструментирование составных вызовов
#### Добавление метрик
##### Запись метрик Redis
