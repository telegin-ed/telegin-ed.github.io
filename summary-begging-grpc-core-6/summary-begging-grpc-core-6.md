...
https://github.com/Apress/beg-grpc-w-asp.net-core-6?ysclid=m1kpka4pdk665992678  
...
#### Any, Value, Wrappers, даты, время (предопределенные типы)
...
ASP.NET Core 6 для gRPC-web реализует те же функции, что и gRPC, кроме: в клиент браузера не поддерживает клиентский и двунаправленный потоковый сервис (только unary и серверный потоковый сервис, см раздел "Переработка CountryService для приложения браузера").
...
#### Скачать корректную версию Protoc и Protobuf Well-Known Types
...
#### Скачать Improbable gRPC-web библиотеку и Google Protobufs библиотеку
