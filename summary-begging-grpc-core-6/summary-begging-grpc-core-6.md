...
- [Chapter 4. Protobuf](#ch4)
    ...
    - [Определение сообщения](#ch4-3)
        ...
        - [Коллекции](#ch4-3-2)
            - [Списки](#ch4-3-2-1)
            - [Словари](#ch4-3-2-2)
            - [Перечисления](#ch4-3-2-3)
            - [Вложенные типы](#ch4-3-2-4)
            - [Импорт типов](#ch4-3-2-5)
            - [Any, Value, Wrappers, даты, время (предопределенные типы)](#ch4-3-2-6)
                - [Any](#ch4-3-2-6-1)
                - [Wrappers](#ch4-3-2-6-2)
                - [Value](#ch4-3-2-6-3)
                - [Дата и время](#ch4-3-2-6-4)
                - [Байты](#ch4-3-2-6-5)
                - [One of](#ch4-3-2-6-6)
                - [Пустое сообщение](#ch4-3-2-6-7)
                - [Комментарии](#ch4-3-2-6-8)
...
* Token-based authentication: основан на использовании JWT, содержащим информацию о текущем пользователе и выпущенным сторонним identity провайдером, а не самим приложением. Инициатор запроса запрашивает у сервера токен, затем передает его в заголовке HTTP-запроса. Клиент валидирует подлинность токена чтением метаданных, предоставленных сторонним identity-провайдером, проверяется цифровая подпись полученного токена (что данный токен пришел действительно от данного поставщика). Данный типа аутентификации наиболее распространен в веб приложениях.
...
Канал поддерживает опцию максимального количества переданных или принятых сообщений. Подробнее о доступных опциях настройки gRPC: https://grpc.github.io/grpc/core/group__grpc__arg__keys.html.
...
https://learn.microsoft.com/ru-ru/aspnet/core/grpc/client?view=aspnetcore-8.0
...
service CountryService {
    ...
    rpc Create(stream CountryCreateRequest) returns (stream
...
* Коллекции (repeated, map)
* Перечисления (enum)
* Вложенные типы
* Импорт типов (import)
* Any, Value, Wrapper, даты, время (предопределенные типы)
* Байты
* One of
* пустые сообщения
* комментарии
...
Спецификация позволяет резервировать значения перечислений, однако лучше подход с выпуском новой версией API.

#### <a id="ch4-3-2-4">Вложенные типы</a>

Поддерживается вложение определение одних сообщений в другие:
...
#### <a id="ch4-3-2-6">Any, Value, Wrappers, даты, время (предопределенные типы)</a>

Any, Value, Wrapper, дата, время - типы, которые имеют особое поведение и требуют специальной обработки. Они не могут быть скомпилированы как есть в C#. Поэтому Google предоставляет специальные proto-файлы которые должны быть добавлены и во время компиляции .NET интерпретирует их с C# расширением, которое известно как расширение Protobuf's Well-Known Types.

##### <a id="ch4-3-2-6-1">Any</a>
...
##### <a id="ch4-3-2-6-2">Wrappers</a>

Nullable-типы в C# не поддерживаютс в Protobuf (они получают значения по-умолчанию). Для использованию nullable-типов используются обертки Well-Known Type wrappers.
...
##### <a id="ch4-3-2-6-4">Дата и время</a>
...
##### <a id="ch4-3-2-6-7">Пустое сообщение</a>
...
Пример использования Well-Known типа Empty. Необходимо импортировать путь "google/protobuf/Empty.proto", Protoc знает его и не требует сохранения дополнительных файлов.
...
##### <a id="ch4-3-2-6-8">Комментарии</a>
...
### <a id="appendix-1">Локальная отладка</a>
